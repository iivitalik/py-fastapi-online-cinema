...
@router.post("/like")
def like_movie(movie_uuid: str, is_like: bool, db: Session = Depends(database.get_db), current_user = Depends(oauth2.get_current_user)):
...
@router.post("/rate")
def rate_movie(movie_uuid: str, rating: int, db: Session = Depends(database.get_db), current_user = Depends(oauth2.get_current_user)):
    if not (1 <= rating <= 10):
...
def add_to_favorites(movie_uuid: str, db: Session = Depends(database.get_db), current_user = Depends(oauth2.get_current_user)):
    movie = db.query(models.Movie).filter(models.Movie.uuid == movie_uuid).first()
